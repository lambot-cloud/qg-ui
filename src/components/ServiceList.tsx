...
import { Service, ServiceWithInfra } from '../types/service';
...
export const ServicesList: React.FC<ServicesListProps> = ({ services }) => {
  ...
  const servicesWithInfra = services as unknown as ServiceWithInfra[];
  ...
  const platforms = useMemo(() => {
    const uniquePlatforms = new Set(servicesWithInfra.map(s => s.platform).filter(Boolean));
    return ['all', ...Array.from(uniquePlatforms)];
  }, [servicesWithInfra]);

  const zones = useMemo(() => {
    const uniqueZones = new Set(servicesWithInfra.map(s => s.zone).filter(Boolean));
  ...
  const handleTextFieldChange = (filterName: keyof Filters) => (
  ...
  const handleSelectChange = (filterName: keyof Filters) => (
  ...
  const filteredAndSortedServices = useMemo(() => {
    ...
    return result;
  }, [servicesWithInfra, filters, sortConfig]);
